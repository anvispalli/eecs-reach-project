...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    // Exits if game file doesn't open
    if (!gameFile.is_open()) {
        exit(1);
    }
    
    // Begins the game with AI, start prompt, and file load
    isAIMode = isAIModeIn;
    ...
    string newLine;
    while (gameFile >> newLine) {
        
        Person p(newLine);
        int pSpawn = p.getTurn();
        
        while (building.getTime() < pSpawn) {
    ...
    // After all files are read and all people are spawned, game is not over
    while (true) {
        building.prettyPrintBuilding(cout);
        satisfactionIndex.printSatisfaction(cout, false);
        checkForGameEnd();
...
bool Game::isValidPickupList(const string& pickupList, 
    ...
    // Check to make sure string uses digits
    ...
    // Check for correct capacity
    if (pickupList.size() > ELEVATOR_CAPACITY) {
        return false;
    }
    
    // Check to see if there are duplicate people
    for (int i = 0; i < pickupList.size(); i++) {
    ...
    bool firstPersonDirection = true;
    ...
    for (int i = 0; i < pickupList.size(); i++) {
        ...
        Person p = floor.getPersonByIndex(index);
        int personTarget = p.getTargetFloor();
        bool pDirectionUp = (personTarget > pickupFloorNum);
        
        if (firstCheck) {
            firstPersonDirection = pDirectionUp;
            firstCheck = false;
        }
        else if (pDirectionUp != firstPersonDirection) {
            return false;
        }
    }
    return true;
